...
from src.preprocess.codegen.java_processor import JAVA_CHAR2TOKEN, JAVA_TOKEN2CHAR
from src.preprocess.codegen.tokenization_utils import ind_iter
...
def clean_hashtags_function(function):
    ...
    function = re.sub("[#][ ][i][f][ ][d][e][f][i][n][e][d][ ][(][ ].*?[ ][)]", "", function)
...
class CppProcessor(TreeSitterLangProcessor):
    ...
    def extract_functions(self, code):
        ...
        try:
            code = clean_hashtags_function(code)
            code = code.replace("ENDCOM", "\n").replace("▁", "SPACETOKEN").replace(NEW_LINE, "\n")
                ...
                if token == ")" and (
                    ...
                    if token == "{":
                        ...
                        if (
                            ...
                            function = clean_hashtags_function(function)
                            function = function.strip()
