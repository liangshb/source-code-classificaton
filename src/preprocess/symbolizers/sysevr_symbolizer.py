...
xread = xlrd.open_workbook("symbolizers/function.xls")
keywords_4 = []
...
function = r"^[_a-zA-Z][_a-zA-Z0-9]*$"
variable = r"^[_a-zA-Z][_a-zA-Z0-9(->)?(\.)?]*$"
number = r"[0-9]+"
stringConst = r'(^\'[\s|\S]*\'$)|(^"[\s|\S]*"$)'
constValue = ["NULL", "false", "true", "nullptr"]
phla = r"[^a-zA-Z0-9_]"
space = r"\s"
spa = r""


def iskeyword_3(token):
...
def create_variable(string, token):
    length = len(string)
    stack1 = []
    s = ""
    i = 0
    ...
    while index < len(list_code):
        ...
        while j < len(token):
            if token[j] in constValue:
                ...
                if (
                    (token[j] in keywords_0)
                    or (token[j] in typewords_0)
                    or (token[j] in typewords_1 or token[j] in typewords_2)
                ):
                    j += 1
...
def symbolize_fn(example, tokens_key: str = "tokens", code_sym_key: str = "code-sym"):
    symbolized_code_list = []
    for lines_tokens in example[tokens_key]:
        code = mapping(lines_tokens)
        symbolized_code_list.append(code)

    return {code_sym_key: symbolized_code_list}
